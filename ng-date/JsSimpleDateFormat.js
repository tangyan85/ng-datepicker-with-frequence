...
(function() {
	...
	ltr.w.__extends__(ltr.D, {
		...
		getWeek: function() {
			//get exactly same weeknum of MS Excel function WEEKNUM!!!
			this.dt.setHours(0, 0, 0);
			var today = new Date(this.dt), ws = new Date(this.dt), thu = new Date(this.dt), we = new Date(this.dt);
			ws.setDate(ws.getDate() - (ws.getDay() || 7));
			we.setDate(we.getDate() + 7 - (we.getDay() || 7));
			thu.setDate(thu.getDate() + 4 - (thu.getDay() || 7));
			var thisYearStart = new Date(today.getFullYear(), 0, 1);
			var firstThursday = new Date(thisYearStart.getTime() - (thisYearStart.getDay() || 7) * 864e5 + 4 * 864e5);
			var firstSunday = new Date(thisYearStart.getTime() - (thisYearStart.getDay() || 7) * 864e5 + 7 * 864e5);
			var isoWeekNum = Math.ceil(((thu - firstThursday) / 864e5) / 7);

			if(thisYearStart.getDay() || 7 == 1)
				return isoWeekNum + 1;
			else
				return thisYearStart.getDay() || 7 <= 4 ? isoWeekNum : isoWeekNum + 1;
...
//			return ((this.dt.getDate() < iDay) ? (iWeek+1) : iWeek);
		},
		isValidVal: function(iVal) {
